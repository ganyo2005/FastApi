...
def vote(vote:schema.Vote, db:Session=Depends(database.get_db), current_user :int=Depends(Oauth2.get_current_user)):
            votes_qurey=db.query(models.Votes).filter(models.Votes.post_id==vote.post_id,models.Votes.user_id==current_user.id)
            post=db.query(models.Post).filter(models.Post.id==vote.post_id).first()
            found_vote=votes_qurey.first()
            if not post:
            ...
            if(vote.dir==1):
                    ...
                    votes_qurey.delete(synchronize_session=False)
                    db.commit()
